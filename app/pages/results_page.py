...
counts = dataset.groupby(['name', 'search_date']).count().sort_values(by=["name", "search_date"], ascending=True)
counts["Number of properties"] = counts["price"]

fig, axs = plt.subplots(1, 1, figsize=(10, 7.5))
sns.lineplot(data=counts, x="search_date", y="Number of properties", hue="name", style="name", markers=True, ax=axs, sort=True)
